...
as $$
  ...
  select jsonb_build_object(
    ...
    jsonb_build_object(
      ...
      'targetAdherence',
        case
          when (select delivered from summary) > 0
          then ((select target_total from target_summary) / (select delivered from summary)) * 100
          else 0
        end
    ),
    'byTurno',
    ...
    coalesce(
      (
        select jsonb_agg(jsonb_build_object('date', date, 'delivered', delivered, 'pedidos', pedidos) order by date)
        from by_day
      ),
      '[]'::jsonb
    ),
